...
<head>
    ...
    <style>
        ...
        .modal {
            ...
            /* Using display: none by default, controlled by JS */
        ...
        .modal-content {
            ...
            /* Removed transform from animation to prevent zoom issues */
            animation: fadeInOnly 0.3s ease-out forwards; /* New animation for fade only */
            color: #e0f2fe;
            text-align: left;
        ...
        /* Animations */
        @keyframes fadeInOnly { /* New animation for modal content */
            from { opacity: 0; }
            to { opacity: 1; }
        }
        @keyframes fadeIn {
...
<body>
    ...
    <script>
        ...
        const studentDetails = {
            ...
            "Syifa Baru Pindahan": { fullName: "SYIFA (BARU Pindahan)", gender: "Perempuan", role: "Murid" }, // New Syifa with updated name
        };
        ...
        const modalContent = modal.querySelector('.modal-content'); // Get modal content element
        ...
        function openModal(nameKey) {
            ...
            // Clean up the displayed name for the modal title
            ...
            if (studentData) {
                ...
                // Fallback if nameKey somehow not found (shouldn't happen with comprehensive studentDetails)
                modalFullName.textContent = displayName;
                modalGender.textContent = "Tidak diketahui";
                modalRole.textContent = "Murid";
            }

            // Prevent body scrolling when modal is open to avoid layout shifts
        ...
        function closeModal() {
            ...
            modal.addEventListener('transitionend', function handler(event) {
                // Ensure the transition is for opacity on the modal itself
                if (event.propertyName === 'opacity' && event.target === modal) {
                    modal.style.display = 'none';
                    // Restore body scrolling after modal is fully hidden
